...
- **`buildGraph.php`** — парсит PHP, JS/TS, Go и Astro файлы, строит граф зависимостей в SQLite. Инкрементальный: повторный запуск обновляет только изменённые файлы (~100–200 мс). Если настроены эмбеддинги — автоматически индексирует новые символы.
- **`claudeSearch.php`** — CLI-интерфейс. Перед `graph`/`similar`-запросами автоматически вызывает `buildGraph.php`.
- **`embed.php`** — провайдеры векторных эмбеддингов для семантического поиска (`similar`). Поддерживает Voyage AI, OpenAI и Ollama. Подключается только если задан `CS_EMBED_PROVIDER` в `config.php`.
...
claudeSearch/
  ...
  parsers/
    ...
    js.php              — парсер JS/JSX/TS/TSX (import, компоненты, функции, export function)
    go.php              — парсер Go (struct, interface, func, receivers, import)
    astro.php           — парсер Astro (TypeScript frontmatter + JSX-шаблон)
```

SQLite-граф хранится в `claudeSearch/code_graph.sqlite` проекта (путь задаётся в `config.php`).
...
Положи папку `claudeSearch/` в удобное место внутри проекта (или рядом с ним, на том же уровне).

Дальнейшую настройку можно выполнить двумя способами:

**Вариант A — попросить AI-агента.** Если у вас уже настроен AI-ассистент (Claude Code, Cursor и т.д.), скажите ему:

> Установи claudeSearch: папка уже в проекте. Настрой `config.php` под структуру этого репозитория (пути к директориям, имя БД, расширения файлов) и добавь `claudeSearch/code_graph.sqlite` в `.gitignore`.

Агент изучит структуру проекта и заполнит все настройки самостоятельно.

**Вариант B — вручную.** Следуй шагам 2–7 ниже.
...
Папка `claudeSearch/` должна быть доступна на запись.

### 3. Поправить `$rootDir` при необходимости

По умолчанию `realpath(__DIR__ . '/../')` указывает на **родителя** папки `claudeSearch/`. Измени `/../`, если папка размещена иначе.

### 4. Добавить новый язык / включить TypeScript или Astro
...
**TypeScript / Astro проекты** — используй `$scanExts` для маппинга нескольких расширений на один парсер:
```php
$scanDirs = [
    'js'    => [$rootDir . 'src'],   // JS-парсер обрабатывает и TS/TSX
    'astro' => [$rootDir . 'src'],   // Astro-парсер для .astro файлов
];
$scanExts = [
    'js'    => ['ts', 'tsx', 'js', 'jsx'],
    'astro' => ['astro'],
];
$extensions = ['ts', 'tsx', 'astro', 'css'];  // для текстового поиска
```
...
```bash
php claudeSearch/buildGraph.php          # инкрементальный (только изменённые файлы)
php claudeSearch/buildGraph.php --full   # полная пересборка графа
```
...
**Astro:** TypeScript frontmatter (между `---`) парсится как JS/TS — импорты, функции, константы; `<Component>` в шаблоне индексируются как jsx-рефы. Включается через `$scanExts = ['astro' => ['astro']]` в `config.php`.
...
**Инструмент поиска:** `claudeSearch/claudeSearch.php`
bash /path/to/project/claudeSearch/cs.sh <action> <term>

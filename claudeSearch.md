...
**Автоматическое разрешение для Claude Code** — добавь в `.claude/settings.json` проекта разрешение для `cs.sh` (только эту строку, без широких bash-разрешений):
```json
{
  "permissions": {
    "allow": [
...
- Граф хранится в `cache/code_graph.sqlite` (путь задаётся в `config.php`), добавь в `.gitignore`

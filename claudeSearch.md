...
- **`buildGraph.php`** — парсит PHP, JS и Go файлы, строит граф зависимостей в SQLite (`code_graph.sqlite`). Инкрементальный: повторный запуск обновляет только изменённые файлы. Если настроены эмбеддинги — автоматически индексирует новые символы.
